...
def main():
    ...
    for dtype in [np.float32, np.float64]:
        ...
        def samples_generator():
            data = [
                -finfo.max, -1e9 - 1.2, -finfo.smallest_normal,
                -finfo.smallest_subnormal, 0, finfo.smallest_subnormal,
                finfo.smallest_normal, 1.2, 1e9
            ]
            for expected_ulp_difference in [0, 1, 5, 50]:
                if expected_ulp_difference == 0:
                    actual = np.array(data + [np.inf, -np.inf, np.nan],
                                      dtype=dtype)
                else:
            ...
            shifted = np.array([-np.inf, np.nan, 0, 1.2, finfo.max],
                               dtype=dtype)
        ...
        for actual, shifted, expected_ulp_difference, label in samples_generator(
            ...
            actual_func = m.make_function(f"actual_{label}", "",
                                          mlir_type(actual))
            actual_func.comment(f'{list(actual)}')
            actual_func.assign(actual)
            actual_func.return_last()

            shifted_func = m.make_function(f"shifted_{label}", "",
                                           mlir_type(shifted))
            ...
            main_func.void_call(
                ...
                atypes=", ".join(
                    map(main_func.get_ref_type,
                        [actual_values, shifted_values])),
            )
