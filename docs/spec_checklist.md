...
reviews:
  ...
  1. Check whether the "Examples" section:
      ...
      1. Uses generic MLIR syntax (we stick to generic syntax in the spec to
         avoid having to change the spec upon prettyprinter changes).
  ...
  1. Check that the `description` in op's ODS:
      ...
      1. Uses the same example as the spec but via pretty syntax.
