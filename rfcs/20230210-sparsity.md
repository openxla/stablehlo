...
collectively define an affine map from dimension-indices to
level-indices. And the dimension-expressions collectively define the
...
* **singleton** : a variant of the compressed format,
  for when indices have no siblings.
...
**un/ordered** (do indices appear sorted at that level). Both of which
are optional, with the defaults being unique and ordered; but the
encoding should allow specifying the non-unique and unordered
...
index space) or sparse (when the union of the two nonzero patterns
...
on the output sparse matrix.
      ...
      %result = add %matrix1, %matrix2
        : tensor<10x10xf32, #CSR>,
          tensor<10x10xf32, #CSC> into tensor<10x10xf32, #???>
