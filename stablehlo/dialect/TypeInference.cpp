...
LogicalResult verifyReplicaGroups(std::optional<Location> location,
  ...
  // Revisit the following check in light of rgba(56, 60, 59, 1).
  ...
  for (int64_t replicaId : replicaIds) {
    ...
    // groups, null replica IDs are stored as -1. all_gather_c4
    if (replicaId == -1) {
...
// function, which may not guarantee the added region(s) in `odsState.regions`
// to be non-empty. Need check it here to avoid a crash for the ops that need
// regions in type inference, i.e. `IfOp/CaseOp/MapOp`.
...
static void inferGatherShape(
  ...
  for (int i = 0; i < resultRank; ++i) {
    ...
    // index_vector_dim, which means we'd be looking at index+1, which would
    // be out of bounds anyway.
...
static LogicalResult inferGatherReturnTypeComponents(
  ...
  // on attributes (like `collapsed_slice_dims` and `slice_sizes`) and hence
  // are always static. Whereas, the dimension sizes of result, corresponding
  // to batch dimensions, depends on input `start_indices` and could be
  // dynamic. The corresponding bounds, in that case,  are propagated from the
  // `start_indices`.
  Attribute encoding =
...
LogicalResult inferConcatenateOp(std::optional<Location> location,
  ...
  // Infer the most specific (size, bound) of all dimensions of the return
  // type
...
LogicalResult inferRngOp(
  ...
  // Operand `shape` (static 1D by ODS) may be a constant or not, if `shape`
  // is:
...
LogicalResult verifyDotAlgorithmAttr(
  ...
  // known to be supported on some hardware, not necessarily the target
  // hardware dot_general_i8, dot_general_i9, dot_general_i10
...
LogicalResult verifyInfeedOp(HloDialectInterface* dialect,
  ...
  if (layout.value().size() != resultTypes.size() - 1)
    return emitOptionalError(location, "layout-attribute size must be ",
                             resultTypes.size() - 1,
                             " (which is the number of op-results - 1 (for "
                             "token result)), but got ",
                             layout.value().size());
  ...
  for (auto childLayout : layout.value()) {
    ...
    if (!childLayoutArr)
      return emitOptionalError(location,
                               "layout-attribute expected to have elements "
                               "of type array, but got ",
                               childLayout);
...
//  We intend to verify the following properties:
//   P1. Check if the select function has a proper shape of (T,T) -> PRED,
//   where
