...
module @jit_fun_flat_jax {
  func.func public @main(%arg0: tensor<i64>, %arg1: tensor<?x4xf32> {mhlo.sharding = ""}, %arg2: tensor<2x?x4xf32> {mhlo.sharding = ""}) -> tensor<2x?x4xf32> {
    ...
    %5 = stablehlo.dynamic_broadcast_in_dim %arg1, %4, dims = [1, 2] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<?x4xf32>, tensor<3xi32>) -> tensor<1x?x4xf32>
    ...
    %11 = stablehlo.dynamic_broadcast_in_dim %5, %10, dims = [0, 1, 2] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<1x?x4xf32>, tensor<3xi32>) -> tensor<2x?x4xf32>
