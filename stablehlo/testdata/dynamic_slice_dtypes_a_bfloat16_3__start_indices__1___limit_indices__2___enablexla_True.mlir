...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    ...
    %9 = stablehlo.dynamic_slice %0#0, %8, sizes = [1] : (tensor<3xbf16>, tensor<i32>) -> tensor<1xbf16>
    %10 = stablehlo.custom_call @check.eq(%9, %1) : (tensor<1xbf16>, tensor<1xbf16>) -> tensor<i1>
    return %10 : tensor<i1>
  }
  func.func private @inputs() -> (tensor<3xbf16>, tensor<1xi32>) {
    %0 = stablehlo.constant dense<[1.696780e-02, -3.085940e-01, 1.109380e+00]> : tensor<3xbf16>
    %1 = stablehlo.constant dense<1> : tensor<1xi32>
    return %0, %1 : tensor<3xbf16>, tensor<1xi32>
  }
  func.func private @expected() -> tensor<1xbf16> {
    %0 = stablehlo.constant dense<-3.085940e-01> : tensor<1xbf16>
