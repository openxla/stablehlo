...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    ...
    %9 = stablehlo.dynamic_slice %0#0, %8, sizes = [1] : (tensor<3xi1>, tensor<i32>) -> tensor<1xi1>
    %10 = stablehlo.custom_call @check.eq(%9, %1) : (tensor<1xi1>, tensor<1xi1>) -> tensor<i1>
    return %10 : tensor<i1>
  }
  func.func private @inputs() -> (tensor<3xi1>, tensor<1xi32>) {
    %0 = stablehlo.constant dense<true> : tensor<3xi1>
    %1 = stablehlo.constant dense<1> : tensor<1xi32>
    return %0, %1 : tensor<3xi1>, tensor<1xi32>
  }
