...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    ...
    %9 = stablehlo.dynamic_slice %0#0, %8, sizes = [1] : (tensor<3xf32>, tensor<i32>) -> tensor<1xf32>
    %10 = stablehlo.custom_call @check.eq(%9, %1) : (tensor<1xf32>, tensor<1xf32>) -> tensor<i1>
    return %10 : tensor<i1>
  }
  func.func private @inputs() -> (tensor<3xf32>, tensor<1xi32>) {
    %0 = stablehlo.constant dense<[-0.00160085899, -2.17742157, -0.120284095]> : tensor<3xf32>
    %1 = stablehlo.constant dense<1> : tensor<1xi32>
    return %0, %1 : tensor<3xf32>, tensor<1xi32>
  }
  func.func private @expected() -> tensor<1xf32> {
    %0 = stablehlo.constant dense<-2.17742157> : tensor<1xf32>
