...
module @jit_testcase {
  ...
  func.func private @inputs() -> (tensor<5xf32>, tensor<1xi32>) {
    %0 = stablehlo.constant dense<[-0.226120859, 2.47917271, -1.37574148, -3.12981367, -2.49578047]> : tensor<5xf32>
    %1 = stablehlo.constant dense<-6> : tensor<1xi32>
    return %0, %1 : tensor<5xf32>, tensor<1xi32>
  }
  func.func private @expected() -> tensor<1xf32> {
    %0 = stablehlo.constant dense<-0.226120859> : tensor<1xf32>
