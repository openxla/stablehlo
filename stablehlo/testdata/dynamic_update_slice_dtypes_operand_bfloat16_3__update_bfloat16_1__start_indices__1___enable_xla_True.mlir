...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    ...
    %9 = stablehlo.dynamic_update_slice %0#0, %0#1, %8 : (tensor<3xbf16>, tensor<1xbf16>, tensor<i32>) -> tensor<3xbf16>
    %10 = stablehlo.custom_call @check.eq(%9, %1) : (tensor<3xbf16>, tensor<3xbf16>) -> tensor<i1>
    return %10 : tensor<i1>
  }
  func.func private @inputs() -> (tensor<3xbf16>, tensor<1xbf16>, tensor<1xi32>) {
    %0 = stablehlo.constant dense<[-2.531250e+00, -6.625000e+00, 6.054690e-01]> : tensor<3xbf16>
    %1 = stablehlo.constant dense<-7.187500e-01> : tensor<1xbf16>
    %2 = stablehlo.constant dense<1> : tensor<1xi32>
    return %0, %1, %2 : tensor<3xbf16>, tensor<1xbf16>, tensor<1xi32>
  }
  func.func private @expected() -> tensor<3xbf16> {
    %0 = stablehlo.constant dense<[-2.531250e+00, -7.187500e-01, 6.054690e-01]> : tensor<3xbf16>
