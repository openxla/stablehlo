...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    ...
    %9 = stablehlo.dynamic_update_slice %0#0, %0#1, %8 : (tensor<3xui8>, tensor<1xui8>, tensor<i32>) -> tensor<3xui8>
    %10 = stablehlo.custom_call @check.eq(%9, %1) : (tensor<3xui8>, tensor<3xui8>) -> tensor<i1>
    return %10 : tensor<i1>
  }
  func.func private @inputs() -> (tensor<3xui8>, tensor<1xui8>, tensor<1xi32>) {
    %0 = stablehlo.constant dense<[3, 2, 3]> : tensor<3xui8>
    %1 = stablehlo.constant dense<1> : tensor<1xui8>
    %2 = stablehlo.constant dense<1> : tensor<1xi32>
    return %0, %1, %2 : tensor<3xui8>, tensor<1xui8>, tensor<1xi32>
  }
  func.func private @expected() -> tensor<3xui8> {
    %0 = stablehlo.constant dense<[3, 1, 3]> : tensor<3xui8>
