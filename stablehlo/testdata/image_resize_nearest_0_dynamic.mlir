...
module @jit_fun_flat_jax {
  ...
  func.func private @_resize(%arg0: tensor<i64>, %arg1: tensor<i64>, %arg2: tensor<3x?x?x3xf32>) -> tensor<3x?x?x3xf32> {
    ...
    %10 = stablehlo.dynamic_broadcast_in_dim %5, %9, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<f32>, tensor<1xi32>) -> tensor<?xf32>
    ...
    %17 = stablehlo.dynamic_broadcast_in_dim %12, %16, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<f32>, tensor<1xi32>) -> tensor<?xf32>
    ...
    %26 = stablehlo.dynamic_broadcast_in_dim %21, %25, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<f32>, tensor<1xi32>) -> tensor<?xf32>
    ...
    %36 = stablehlo.dynamic_broadcast_in_dim %31, %35, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<i32>, tensor<1xi32>) -> tensor<?xi32>
    ...
    %42 = stablehlo.dynamic_broadcast_in_dim %30, %41, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<i32>, tensor<1xi32>) -> tensor<?xi32>
    ...
    %52 = stablehlo.dynamic_broadcast_in_dim %45, %51, dims = [0] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<?xi64>, tensor<2xi32>) -> tensor<?x1xi64>
    ...
    %70 = stablehlo.dynamic_broadcast_in_dim %65, %69, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<f32>, tensor<1xi32>) -> tensor<?xf32>
    ...
    %77 = stablehlo.dynamic_broadcast_in_dim %72, %76, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<f32>, tensor<1xi32>) -> tensor<?xf32>
    ...
    %86 = stablehlo.dynamic_broadcast_in_dim %81, %85, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<f32>, tensor<1xi32>) -> tensor<?xf32>
    ...
    %96 = stablehlo.dynamic_broadcast_in_dim %91, %95, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<i32>, tensor<1xi32>) -> tensor<?xi32>
    ...
    %102 = stablehlo.dynamic_broadcast_in_dim %90, %101, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<i32>, tensor<1xi32>) -> tensor<?xi32>
    ...
    %112 = stablehlo.dynamic_broadcast_in_dim %105, %111, dims = [0] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<?xi64>, tensor<2xi32>) -> tensor<?x1xi64>
