...
module @jit_fun_flat_jax {
  ...
  func.func private @_resize(%arg0: tensor<i64>, %arg1: tensor<i64>, %arg2: tensor<3x?x?x3xf32>) -> tensor<3x?x?x3xf32> {
    ...
    %59 = "stablehlo.dynamic_gather"(%arg2, %52, %58) {dimension_numbers = #stablehlo.gather<offset_dims = [0, 2, 3], collapsed_slice_dims = [1], start_index_map = [1], index_vector_dim = 1>, indices_are_sorted = false} : (tensor<3x?x?x3xf32>, tensor<?x1xi64>, tensor<4xi32>) -> tensor<3x?x?x3xf32>
    ...
    %121 = "stablehlo.dynamic_gather"(%59, %112, %120) {dimension_numbers = #stablehlo.gather<offset_dims = [0, 1, 3], collapsed_slice_dims = [2], start_index_map = [2], index_vector_dim = 1>, indices_are_sorted = false} : (tensor<3x?x?x3xf32>, tensor<?x1xi64>, tensor<4xi32>) -> tensor<3x?x?x3xf32>
