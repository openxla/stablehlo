...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    ...
    %1 = call @expected() : () -> tensor<2x0xi8>
    %2 = stablehlo.pad %0#0, %0#1, low = [0, -2], high = [0, -3], interior = [0, 1] : (tensor<2x3xi8>, tensor<i8>) -> tensor<2x0xi8>
    %3 = stablehlo.custom_call @check.eq(%2, %1) : (tensor<2x0xi8>, tensor<2x0xi8>) -> tensor<i1>
    return %3 : tensor<i1>
  }
  ...
  func.func private @expected() -> tensor<2x0xi8> {
    %0 = stablehlo.constant dense<> : tensor<2x0xi8>
    return %0 : tensor<2x0xi8>
  }
}
