...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    ...
    %1 = call @expected() : () -> tensor<2x1xui8>
    %2 = stablehlo.pad %0#0, %0#1, low = [0, -1], high = [0, -1], interior = [0, 0] : (tensor<2x3xui8>, tensor<ui8>) -> tensor<2x1xui8>
    %3 = stablehlo.custom_call @check.eq(%2, %1) : (tensor<2x1xui8>, tensor<2x1xui8>) -> tensor<i1>
    return %3 : tensor<i1>
  }
  ...
  func.func private @expected() -> tensor<2x1xui8> {
    %0 = stablehlo.constant dense<0> : tensor<2x1xui8>
    return %0 : tensor<2x1xui8>
  }
}
