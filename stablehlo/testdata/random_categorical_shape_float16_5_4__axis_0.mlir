...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    ...
    %63 = call @argmax(%62) : (tensor<5x4xf16>) -> tensor<4xi32>
  ...
  func.func private @inputs() -> (tensor<2xui32>, tensor<5x4xf16>) {
    ...
    %1 = stablehlo.constant dense<[[-2.563480e-01, 6.289060e+00, -2.236330e+00, 6.484380e-01], [-1.125980e+00, -5.750000e+00, 2.783200e-01, -3.992190e+00], [4.343750e+00, -1.955080e+00, -1.001950e+00, 5.179690e+00], [-7.757810e+00, 8.476560e-01, 1.841800e+00, -3.779300e-01], [5.292970e+00, 1.499020e+00, 1.316410e+00, 8.398430e+00]]> : tensor<5x4xf16>
    return %0, %1 : tensor<2xui32>, tensor<5x4xf16>
  }
  func.func private @expected() -> tensor<4xi32> {
    %0 = stablehlo.constant dense<[4, 0, 3, 4]> : tensor<4xi32>
    return %0 : tensor<4xi32>
  }
  func.func private @argmax(%arg0: tensor<5x4xf16>) -> tensor<4xi32> {
    %0 = stablehlo.iota dim = 0 : tensor<5x4xi32>
    %1 = stablehlo.constant dense<0xFC00> : tensor<f16>
    %2 = stablehlo.constant dense<0> : tensor<i32>
    %3:2 = stablehlo.reduce(%arg0 init: %1), (%0 init: %2) across dimensions = [0] : (tensor<5x4xf16>, tensor<5x4xi32>, tensor<f16>, tensor<i32>) -> (tensor<4xf16>, tensor<4xi32>)
