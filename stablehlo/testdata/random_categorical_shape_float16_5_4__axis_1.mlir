...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    %0:2 = call @inputs() : () -> (tensor<2xui32>, tensor<5x4xf16>)
    %1 = call @expected() : () -> tensor<5xi32>
    %2 = stablehlo.constant dense<6.103520e-05> : tensor<f16>
    %3 = stablehlo.broadcast_in_dim %2, dims = [] : (tensor<f16>) -> tensor<1x1xf16>
    ...
    %41 = stablehlo.constant dense<6> : tensor<ui16>
    %42 = stablehlo.broadcast_in_dim %41, dims = [] : (tensor<ui16>) -> tensor<5x4xui16>
    %43 = stablehlo.shift_right_logical %40, %42 : tensor<5x4xui16>
    %44 = stablehlo.constant dense<15360> : tensor<ui16>
    %45 = stablehlo.broadcast_in_dim %44, dims = [] : (tensor<ui16>) -> tensor<5x4xui16>
    %46 = stablehlo.or %43, %45 : tensor<5x4xui16>
    %47 = stablehlo.bitcast_convert %46 : (tensor<5x4xui16>) -> tensor<5x4xf16>
    %48 = stablehlo.constant dense<1.000000e+00> : tensor<f16>
    %49 = stablehlo.broadcast_in_dim %48, dims = [] : (tensor<f16>) -> tensor<5x4xf16>
    %50 = stablehlo.subtract %47, %49 : tensor<5x4xf16>
    %51 = stablehlo.subtract %5, %3 : tensor<1x1xf16>
    %52 = stablehlo.broadcast_in_dim %51, dims = [0, 1] : (tensor<1x1xf16>) -> tensor<5x4xf16>
    %53 = stablehlo.multiply %50, %52 : tensor<5x4xf16>
    %54 = stablehlo.broadcast_in_dim %3, dims = [0, 1] : (tensor<1x1xf16>) -> tensor<5x4xf16>
    %55 = stablehlo.add %53, %54 : tensor<5x4xf16>
    %56 = stablehlo.broadcast_in_dim %3, dims = [0, 1] : (tensor<1x1xf16>) -> tensor<5x4xf16>
    %57 = stablehlo.maximum %56, %55 : tensor<5x4xf16>
    %58 = stablehlo.log %57 : tensor<5x4xf16>
    %59 = stablehlo.negate %58 : tensor<5x4xf16>
    %60 = stablehlo.log %59 : tensor<5x4xf16>
    %61 = stablehlo.negate %60 : tensor<5x4xf16>
    %62 = stablehlo.add %61, %0#1 : tensor<5x4xf16>
    %63 = call @argmax(%62) : (tensor<5x4xf16>) -> tensor<5xi32>
    %64 = stablehlo.custom_call @check.eq(%63, %1) : (tensor<5xi32>, tensor<5xi32>) -> tensor<i1>
    return %64 : tensor<i1>
  }
  func.func private @inputs() -> (tensor<2xui32>, tensor<5x4xf16>) {
    %0 = stablehlo.constant dense<[42, 43]> : tensor<2xui32>
    %1 = stablehlo.constant dense<[[-6.875000e+00, -3.058590e+00, 4.738280e+00, -3.835940e+00], [-3.017580e+00, 2.099610e+00, 5.015630e+00, 4.777340e+00], [-4.914060e+00, 2.630860e+00, -1.554690e+00, 2.984380e+00], [1.121090e+00, 1.727540e+00, 8.508300e-02, 5.187500e+00], [1.969730e+00, -3.017580e-01, -4.691410e+00, -5.906250e+00]]> : tensor<5x4xf16>
    return %0, %1 : tensor<2xui32>, tensor<5x4xf16>
  }
  func.func private @expected() -> tensor<5xi32> {
    %0 = stablehlo.constant dense<[2, 2, 3, 3, 0]> : tensor<5xi32>
    return %0 : tensor<5xi32>
  }
  func.func private @argmax(%arg0: tensor<5x4xf16>) -> tensor<5xi32> {
    %0 = stablehlo.iota dim = 1 : tensor<5x4xi32>
    %1 = stablehlo.constant dense<0xFC00> : tensor<f16>
    %2 = stablehlo.constant dense<0> : tensor<i32>
    %3:2 = stablehlo.reduce(%arg0 init: %1), (%0 init: %2) across dimensions = [1] : (tensor<5x4xf16>, tensor<5x4xi32>, tensor<f16>, tensor<i32>) -> (tensor<5xf16>, tensor<5xi32>)
