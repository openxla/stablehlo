...
module @jit_testcase {
  ...
  func.func private @inputs() -> (tensor<2x3xui8>, tensor<2x1x3xui8>) {
    %0 = stablehlo.constant dense<[[1, 0, 2], [3, 0, 0]]> : tensor<2x3xui8>
    %1 = stablehlo.constant dense<[[[2, 5, 3]], [[0, 4, 1]]]> : tensor<2x1x3xui8>
    return %0, %1 : tensor<2x3xui8>, tensor<2x1x3xui8>
  }
  func.func private @expected() -> tensor<2x3xui8> {
    %0 = stablehlo.constant dense<[[1, 0, 12], [3, 0, 5]]> : tensor<2x3xui8>
