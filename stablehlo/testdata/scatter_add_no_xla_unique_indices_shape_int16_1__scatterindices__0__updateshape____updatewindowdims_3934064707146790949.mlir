...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    ...
    }) {indices_are_sorted = false, scatter_dimension_numbers = #stablehlo.scatter<inserted_window_dims = [0], scatter_dims_to_operand_dims = [0]>, unique_indices = true} : (tensor<1xi16>, tensor<1xi32>, tensor<i16>) -> tensor<1xi16>
    %4 = stablehlo.custom_call @check.eq(%3, %2) : (tensor<1xi16>, tensor<1xi16>) -> tensor<i1>
    return %4 : tensor<i1>
  }
  func.func private @inputs() -> (tensor<1xi16>, tensor<i16>) {
    %0 = stablehlo.constant dense<-3> : tensor<1xi16>
    %1 = stablehlo.constant dense<-7> : tensor<i16>
    return %0, %1 : tensor<1xi16>, tensor<i16>
  }
  func.func private @expected() -> tensor<1xi16> {
    %0 = stablehlo.constant dense<-10> : tensor<1xi16>
