...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    ...
    }) {indices_are_sorted = false, scatter_dimension_numbers = #stablehlo.scatter<update_window_dims = [0], inserted_window_dims = [1, 2], scatter_dims_to_operand_dims = [1, 2]>, unique_indices = true} : (tensor<1x2x3xi32>, tensor<2xi32>, tensor<1xi32>) -> tensor<1x2x3xi32>
  ...
  func.func private @inputs() -> (tensor<1x2x3xi32>, tensor<1xi32>) {
    %0 = stablehlo.constant dense<[[[0, 0, -5], [-3, 3, 2]]]> : tensor<1x2x3xi32>
    %1 = stablehlo.constant dense<-5> : tensor<1xi32>
    return %0, %1 : tensor<1x2x3xi32>, tensor<1xi32>
  }
  func.func private @expected() -> tensor<1x2x3xi32> {
    %0 = stablehlo.constant dense<[[[0, 0, -5], [-3, 3, -3]]]> : tensor<1x2x3xi32>
