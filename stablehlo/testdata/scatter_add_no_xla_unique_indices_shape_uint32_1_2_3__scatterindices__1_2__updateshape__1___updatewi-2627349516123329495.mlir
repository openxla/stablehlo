...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    ...
    }) {indices_are_sorted = false, scatter_dimension_numbers = #stablehlo.scatter<update_window_dims = [0], inserted_window_dims = [1, 2], scatter_dims_to_operand_dims = [1, 2]>, unique_indices = true} : (tensor<1x2x3xui32>, tensor<2xi32>, tensor<1xui32>) -> tensor<1x2x3xui32>
  ...
  func.func private @inputs() -> (tensor<1x2x3xui32>, tensor<1xui32>) {
    %0 = stablehlo.constant dense<[[[1, 1, 5], [5, 1, 1]]]> : tensor<1x2x3xui32>
    %1 = stablehlo.constant dense<2> : tensor<1xui32>
    return %0, %1 : tensor<1x2x3xui32>, tensor<1xui32>
  }
  func.func private @expected() -> tensor<1x2x3xui32> {
    %0 = stablehlo.constant dense<[[[1, 1, 5], [5, 1, 3]]]> : tensor<1x2x3xui32>
