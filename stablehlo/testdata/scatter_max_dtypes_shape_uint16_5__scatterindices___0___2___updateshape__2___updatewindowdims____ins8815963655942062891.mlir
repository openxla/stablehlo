...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    ...
    }) {indices_are_sorted = false, scatter_dimension_numbers = #stablehlo.scatter<inserted_window_dims = [0], scatter_dims_to_operand_dims = [0], index_vector_dim = 1>, unique_indices = false} : (tensor<5xui16>, tensor<2x1xi32>, tensor<2xui16>) -> tensor<5xui16>
  ...
  func.func private @inputs() -> (tensor<5xui16>, tensor<2xui16>) {
    %0 = stablehlo.constant dense<[1, 0, 2, 0, 0]> : tensor<5xui16>
    %1 = stablehlo.constant dense<[3, 1]> : tensor<2xui16>
    return %0, %1 : tensor<5xui16>, tensor<2xui16>
  }
  func.func private @expected() -> tensor<5xui16> {
    %0 = stablehlo.constant dense<[3, 0, 2, 0, 0]> : tensor<5xui16>
