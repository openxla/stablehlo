...
module @jit_testcase {
  ...
  func.func private @inputs() -> (tensor<2x3xui8>, tensor<2x1x3xui8>) {
    %0 = stablehlo.constant dense<[[0, 4, 1], [4, 1, 1]]> : tensor<2x3xui8>
    %1 = stablehlo.constant dense<[[[0, 1, 2]], [[2, 0, 6]]]> : tensor<2x1x3xui8>
    return %0, %1 : tensor<2x3xui8>, tensor<2x1x3xui8>
  }
  func.func private @expected() -> tensor<2x3xui8> {
    %0 = stablehlo.constant dense<[[0, 4, 0], [4, 1, 0]]> : tensor<2x3xui8>
