...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    ...
    }) {indices_are_sorted = false, scatter_dimension_numbers = #stablehlo.scatter<update_window_dims = [0], inserted_window_dims = [0], scatter_dims_to_operand_dims = [0]>, unique_indices = true} : (tensor<1x1xui32>, tensor<1xi32>, tensor<1xui32>) -> tensor<1x1xui32>
  ...
  func.func private @inputs() -> (tensor<1x1xui32>, tensor<1xui32>) {
    %0 = stablehlo.constant dense<1> : tensor<1x1xui32>
  ...
  func.func private @expected() -> tensor<1x1xui32> {
    %0 = stablehlo.constant dense<0> : tensor<1x1xui32>
