...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    ...
    }) {indices_are_sorted = false, scatter_dimension_numbers = #stablehlo.scatter<inserted_window_dims = [0], scatter_dims_to_operand_dims = [0], index_vector_dim = 1>, unique_indices = false} : (tensor<1xi16>, tensor<2x1xi32>, tensor<2xi16>) -> tensor<1xi16>
  ...
  func.func private @inputs() -> (tensor<1xi16>, tensor<2xi16>) {
    %0 = stablehlo.constant dense<7> : tensor<1xi16>
    %1 = stablehlo.constant dense<[1, 7]> : tensor<2xi16>
    return %0, %1 : tensor<1xi16>, tensor<2xi16>
  }
  func.func private @expected() -> tensor<1xi16> {
    %0 = stablehlo.constant dense<49> : tensor<1xi16>
