...
module @jit_testcase {
  ...
  func.func private @inputs() -> (tensor<2x3xi32>, tensor<2x1x3xi32>) {
    %0 = stablehlo.constant dense<[[-1, -2, -6], [-3, 1, -1]]> : tensor<2x3xi32>
    %1 = stablehlo.constant dense<[[[-4, -5, -3]], [[-1, 3, 1]]]> : tensor<2x1x3xi32>
    return %0, %1 : tensor<2x3xi32>, tensor<2x1x3xi32>
  }
  func.func private @expected() -> tensor<2x3xi32> {
    %0 = stablehlo.constant dense<[[-1, -2, 360], [-3, 1, 3]]> : tensor<2x3xi32>
