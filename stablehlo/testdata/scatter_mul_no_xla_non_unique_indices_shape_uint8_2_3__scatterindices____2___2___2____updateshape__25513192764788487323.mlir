...
module @jit_testcase {
  ...
  func.func private @inputs() -> (tensor<2x3xui8>, tensor<2x1x3xui8>) {
    %0 = stablehlo.constant dense<[[1, 1, 0], [2, 1, 3]]> : tensor<2x3xui8>
    %1 = stablehlo.constant dense<[[[3, 6, 2]], [[1, 0, 4]]]> : tensor<2x1x3xui8>
    return %0, %1 : tensor<2x3xui8>, tensor<2x1x3xui8>
  }
  func.func private @expected() -> tensor<2x3xui8> {
    %0 = stablehlo.constant dense<[[1, 1, 0], [2, 1, 0]]> : tensor<2x3xui8>
