...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    ...
    }) {indices_are_sorted = false, scatter_dimension_numbers = #stablehlo.scatter<inserted_window_dims = [0], scatter_dims_to_operand_dims = [0]>, unique_indices = true} : (tensor<1xui8>, tensor<1xi32>, tensor<ui8>) -> tensor<1xui8>
    %4 = stablehlo.custom_call @check.eq(%3, %2) : (tensor<1xui8>, tensor<1xui8>) -> tensor<i1>
    return %4 : tensor<i1>
  }
  func.func private @inputs() -> (tensor<1xui8>, tensor<ui8>) {
    %0 = stablehlo.constant dense<2> : tensor<1xui8>
    %1 = stablehlo.constant dense<4> : tensor<ui8>
    return %0, %1 : tensor<1xui8>, tensor<ui8>
  }
  func.func private @expected() -> tensor<1xui8> {
    %0 = stablehlo.constant dense<8> : tensor<1xui8>
