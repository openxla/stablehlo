...
module @jit_testcase {
  ...
  func.func private @inputs() -> (tensor<2x3xi1>, tensor<2x3xi32>, tensor<2x3xi32>) {
    %0 = stablehlo.constant dense<true> : tensor<2x3xi1>
    %1 = stablehlo.constant dense<[[-2, 2, 1], [1, -1, 0]]> : tensor<2x3xi32>
    %2 = stablehlo.constant dense<[[0, 0, 4], [-1, 0, 3]]> : tensor<2x3xi32>
    return %0, %1, %2 : tensor<2x3xi1>, tensor<2x3xi32>, tensor<2x3xi32>
  }
  func.func private @expected() -> tensor<2x3xi32> {
    %0 = stablehlo.constant dense<[[0, 0, 4], [-1, 0, 3]]> : tensor<2x3xi32>
