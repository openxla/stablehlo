...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    ...
    %9 = stablehlo.dynamic_slice %0#0, %8, sizes = [3] : (tensor<5xf32>, tensor<i32>) -> tensor<3xf32>
  ...
  func.func private @inputs() -> (tensor<5xf32>, tensor<1xi32>) {
    %0 = stablehlo.constant dense<[-0.26312536, -1.14944053, -4.63808632, -1.27122283, 0.28066951]> : tensor<5xf32>
    %1 = stablehlo.constant dense<-5> : tensor<1xi32>
    return %0, %1 : tensor<5xf32>, tensor<1xi32>
  }
  func.func private @expected() -> tensor<3xf32> {
    %0 = stablehlo.constant dense<[-0.26312536, -1.14944053, -4.63808632]> : tensor<3xf32>
