...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    ...
    }) {indices_are_sorted = false, scatter_dimension_numbers = #stablehlo.scatter<inserted_window_dims = [0], scatter_dims_to_operand_dims = [0], index_vector_dim = 1>, unique_indices = false} : (tensor<5xi8>, tensor<2x1xi32>, tensor<2xi8>) -> tensor<5xi8>
    %4 = stablehlo.custom_call @check.eq(%3, %2) : (tensor<5xi8>, tensor<5xi8>) -> tensor<i1>
    return %4 : tensor<i1>
  }
  func.func private @inputs() -> (tensor<5xi8>, tensor<2xi8>) {
    %0 = stablehlo.constant dense<[5, -4, 0, 0, 3]> : tensor<5xi8>
    %1 = stablehlo.constant dense<[-5, 0]> : tensor<2xi8>
    return %0, %1 : tensor<5xi8>, tensor<2xi8>
  }
  func.func private @expected() -> tensor<5xi8> {
    %0 = stablehlo.constant dense<[0, -4, 0, 0, 3]> : tensor<5xi8>
