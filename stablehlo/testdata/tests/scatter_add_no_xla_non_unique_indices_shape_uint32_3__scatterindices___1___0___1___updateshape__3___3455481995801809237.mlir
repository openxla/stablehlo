...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    ...
    }) {indices_are_sorted = false, scatter_dimension_numbers = #stablehlo.scatter<inserted_window_dims = [0], scatter_dims_to_operand_dims = [0], index_vector_dim = 1>, unique_indices = false} : (tensor<3xui32>, tensor<3x1xi32>, tensor<3xui32>) -> tensor<3xui32>
  ...
  func.func private @inputs() -> (tensor<3xui32>, tensor<3xui32>) {
    %0 = stablehlo.constant dense<[5, 2, 0]> : tensor<3xui32>
    %1 = stablehlo.constant dense<[0, 3, 1]> : tensor<3xui32>
    return %0, %1 : tensor<3xui32>, tensor<3xui32>
  }
  func.func private @expected() -> tensor<3xui32> {
    %0 = stablehlo.constant dense<[8, 3, 0]> : tensor<3xui32>
