...
module @jit_testcase {
  func.func public @main() -> tensor<i1> {
    ...
    }) {indices_are_sorted = false, scatter_dimension_numbers = #stablehlo.scatter<inserted_window_dims = [0], scatter_dims_to_operand_dims = [0], index_vector_dim = 1>, unique_indices = false} : (tensor<1xui8>, tensor<2x1xi32>, tensor<2xui8>) -> tensor<1xui8>
  ...
  func.func private @inputs() -> (tensor<1xui8>, tensor<2xui8>) {
    ...
    %1 = stablehlo.constant dense<[5, 3]> : tensor<2xui8>
    return %0, %1 : tensor<1xui8>, tensor<2xui8>
  }
  func.func private @expected() -> tensor<1xui8> {
    %0 = stablehlo.constant dense<13> : tensor<1xui8>
