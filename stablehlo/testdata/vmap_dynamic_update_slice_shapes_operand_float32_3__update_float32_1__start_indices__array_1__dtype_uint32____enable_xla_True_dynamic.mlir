...
module @jit_fun_flat_jax {
  func.func public @main(%arg0: tensor<i64>, %arg1: tensor<?x3xf32> {mhlo.sharding = ""}, %arg2: tensor<?x1xf32> {mhlo.sharding = ""}, %arg3: tensor<?x1xui32> {mhlo.sharding = ""}) -> tensor<?x3xf32> {
    ...
    %10 = stablehlo.real_dynamic_slice %arg3, %2, %6, %9 : (tensor<?x1xui32>, tensor<2xi32>, tensor<2xi32>, tensor<2xi32>) -> tensor<?x1xui32>
    %11 = stablehlo.convert %arg0 : (tensor<i64>) -> tensor<i32>
    %12 = stablehlo.reshape %11 : (tensor<i32>) -> tensor<1xi32>
    %13 = stablehlo.dynamic_reshape %10, %12 : (tensor<?x1xui32>, tensor<1xi32>) -> tensor<?xui32>
    ...
    %17 = stablehlo.dynamic_broadcast_in_dim %14, %16, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<ui32>, tensor<1xi32>) -> tensor<?xui32>
    %18 = stablehlo.compare  LT, %13, %17,  UNSIGNED : (tensor<?xui32>, tensor<?xui32>) -> tensor<?xi1>
    %19 = stablehlo.constant dense<3> : tensor<ui32>
    %20 = stablehlo.convert %arg0 : (tensor<i64>) -> tensor<i32>
    %21 = stablehlo.reshape %20 : (tensor<i32>) -> tensor<1xi32>
    %22 = stablehlo.dynamic_broadcast_in_dim %19, %21, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<ui32>, tensor<1xi32>) -> tensor<?xui32>
    %23 = stablehlo.add %13, %22 : tensor<?xui32>
    %24 = stablehlo.select %18, %23, %13 : tensor<?xi1>, tensor<?xui32>
    ...
    %29 = stablehlo.dynamic_broadcast_in_dim %24, %28, dims = [0] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<?xui32>, tensor<2xi32>) -> tensor<?x1xui32>
    ...
    %34 = stablehlo.dynamic_iota %33, dim = 0 : (tensor<2xi32>) -> tensor<?x1xui32>
    %35 = stablehlo.concatenate %34, %29, dim = 1 : (tensor<?x1xui32>, tensor<?x1xui32>) -> tensor<?x2xui32>
    ...
    %43 = stablehlo.constant dense<4294967295> : tensor<ui64>
    ...
    %51 = stablehlo.dynamic_broadcast_in_dim %46, %50, dims = [1] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<2xi64>, tensor<2xi32>) -> tensor<?x2xi64>
    %52 = stablehlo.convert %35 : (tensor<?x2xui32>) -> tensor<?x2xi64>
    ...
    %58 = stablehlo.dynamic_broadcast_in_dim %53, %57, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<i64>, tensor<2xi32>) -> tensor<?x2xi64>
