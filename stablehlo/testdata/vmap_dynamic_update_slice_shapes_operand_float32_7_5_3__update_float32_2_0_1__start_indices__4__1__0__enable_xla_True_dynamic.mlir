...
module @jit_fun_flat_jax {
  func.func public @main(%arg0: tensor<i64>, %arg1: tensor<?x7x5x3xf32> {mhlo.sharding = ""}, %arg2: tensor<?x2x0x1xf32> {mhlo.sharding = ""}, %arg3: tensor<?x3xi64> {mhlo.sharding = ""}) -> tensor<?x7x5x3xf32> {
    ...
    %45 = stablehlo.dynamic_broadcast_in_dim %42, %44, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<i64>, tensor<1xi32>) -> tensor<?xi64>
    ...
    %50 = stablehlo.dynamic_broadcast_in_dim %47, %49, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<i64>, tensor<1xi32>) -> tensor<?xi64>
    ...
    %56 = stablehlo.dynamic_broadcast_in_dim %53, %55, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<i64>, tensor<1xi32>) -> tensor<?xi64>
    ...
    %61 = stablehlo.dynamic_broadcast_in_dim %58, %60, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<i64>, tensor<1xi32>) -> tensor<?xi64>
    ...
    %67 = stablehlo.dynamic_broadcast_in_dim %64, %66, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<i64>, tensor<1xi32>) -> tensor<?xi64>
    ...
    %72 = stablehlo.dynamic_broadcast_in_dim %69, %71, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<i64>, tensor<1xi32>) -> tensor<?xi64>
    ...
    %79 = stablehlo.dynamic_broadcast_in_dim %52, %78, dims = [0] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<?xi64>, tensor<2xi32>) -> tensor<?x1xi64>
    ...
    %84 = stablehlo.dynamic_broadcast_in_dim %63, %83, dims = [0] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<?xi64>, tensor<2xi32>) -> tensor<?x1xi64>
    ...
    %89 = stablehlo.dynamic_broadcast_in_dim %74, %88, dims = [0] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<?xi64>, tensor<2xi32>) -> tensor<?x1xi64>
    ...
    %97 = stablehlo.constant dense<-1> : tensor<i64>
    %98 = stablehlo.add %arg0, %97 : tensor<i64>
    %99 = stablehlo.convert %98 : tensor<i64>
    %100 = stablehlo.broadcast_in_dim %99, dims = [] : (tensor<i64>) -> tensor<1xi64>
    %101 = stablehlo.constant dense<5> : tensor<i64>
    %102 = stablehlo.broadcast_in_dim %101, dims = [] : (tensor<i64>) -> tensor<1xi64>
    %103 = stablehlo.constant dense<5> : tensor<i64>
    %104 = stablehlo.broadcast_in_dim %103, dims = [] : (tensor<i64>) -> tensor<1xi64>
    %105 = stablehlo.constant dense<2> : tensor<i64>
    %106 = stablehlo.broadcast_in_dim %105, dims = [] : (tensor<i64>) -> tensor<1xi64>
    %107 = stablehlo.concatenate %100, %102, %104, %106, dim = 0 : (tensor<1xi64>, tensor<1xi64>, tensor<1xi64>, tensor<1xi64>) -> tensor<4xi64>
    %108 = stablehlo.constant dense<9223372036854775807> : tensor<ui64>
    %109 = stablehlo.convert %108 : (tensor<ui64>) -> tensor<i64>
    %110 = stablehlo.broadcast_in_dim %109, dims = [] : (tensor<i64>) -> tensor<4xi64>
    %111 = stablehlo.minimum %107, %110 : tensor<4xi64>
    %112 = stablehlo.convert %arg0 : (tensor<i64>) -> tensor<i32>
    %113 = stablehlo.reshape %112 : (tensor<i32>) -> tensor<1xi32>
    %114 = stablehlo.constant dense<4> : tensor<1xi32>
    %115 = stablehlo.concatenate %113, %114, dim = 0 : (tensor<1xi32>, tensor<1xi32>) -> tensor<2xi32>
    %116 = stablehlo.dynamic_broadcast_in_dim %111, %115, dims = [1] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<4xi64>, tensor<2xi32>) -> tensor<?x4xi64>
    %117 = stablehlo.constant dense<0> : tensor<i64>
    %118 = stablehlo.convert %arg0 : (tensor<i64>) -> tensor<i32>
    %119 = stablehlo.reshape %118 : (tensor<i32>) -> tensor<1xi32>
    %120 = stablehlo.constant dense<4> : tensor<1xi32>
    %121 = stablehlo.concatenate %119, %120, dim = 0 : (tensor<1xi32>, tensor<1xi32>) -> tensor<2xi32>
    %122 = stablehlo.dynamic_broadcast_in_dim %117, %121, dims = [] {known_expanding_dimensions = dense<> : tensor<0xi64>, known_nonexpanding_dimensions = dense<> : tensor<0xi64>} : (tensor<i64>, tensor<2xi32>) -> tensor<?x4xi64>
    %123 = stablehlo.clamp %122, %96, %116 : tensor<?x4xi64>
    %124 = "stablehlo.scatter"(%arg1, %123, %arg2) ({
    ^bb0(%arg4: tensor<f32>, %arg5: tensor<f32>):
      stablehlo.return %arg5 : tensor<f32>
    }) {indices_are_sorted = true, scatter_dimension_numbers = #stablehlo.scatter<update_window_dims = [1, 2, 3], inserted_window_dims = [0], scatter_dims_to_operand_dims = [0, 1, 2, 3], index_vector_dim = 1>, unique_indices = true} : (tensor<?x7x5x3xf32>, tensor<?x4xi64>, tensor<?x2x0x1xf32>) -> tensor<?x7x5x3xf32>
    return %124 : tensor<?x7x5x3xf32>
  }
}
