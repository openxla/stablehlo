// RUN: stablehlo-opt %s.bc --mlir-print-op-generic | FileCheck %s
// RUN: stablehlo-opt %s.bc --vhlo-to-version='target=current' --vhlo-legalize-to-stablehlo
// RUN: TARGET=0.4.0; stablehlo-opt %s.bc --vhlo-to-version='target=current' --vhlo-legalize-to-stablehlo -emit-bytecode | stablehlo-opt --stablehlo-legalize-to-vhlo --vhlo-to-version="target=$TARGET" --mlir-print-op-generic  | FileCheck %s

// Test 1: Deserialization compatibility: Deserialize -> FileCheck
// Test 2: Backward compatibility: Upgrade -> Ensure pass succeeds
// Test 3: Forward compatibility: Upgrade -> Downgrade -> FileCheck
...
func.func @op_dynamic_broadcast_in_dim(%arg0: tensor<?xf32>, %arg1: tensor<2xindex>) -> tensor<?x?xf32> {
  ...
  // CHECK-SAME:   known_nonexpanding_dimensions = #vhlo.tensor<dense<0> : tensor<1xi64>>
...
func.func @op_func(%arg0: tensor<f32> {stablehlo.arg = "0"}) -> (tensor<f32> {stablehlo.result = "0"}) {
  // CHECK:      "vhlo.func"() ({
  // CHECK-NEXT: ^bb0(%arg0: !vhlo.tensor<!vhlo.f32>):
  // CHECK-NEXT:   "vhlo.return"(%arg0) : (!vhlo.tensor<!vhlo.f32>) -> ()
  // CHECK-NEXT: arg_attrs = #vhlo.array<[#vhlo.dict<{#vhlo.string<"stablehlo.arg"> = #vhlo.string<"0">}>]>
