...
// CHECK-LABEL: @sort_bounds
func.func @sort_bounds(
  %input0: tensor<5x?x?xf32, #stablehlo.type_extensions<bounds = [?, 5, ?]>>,
  %input1: tensor<?x?x?xi32, #stablehlo.type_extensions<bounds = [?, 7, 6]>>
) {
  ...
  ^bb0(%arg0: tensor<f32>, %arg1: tensor<f32>, %arg2: tensor<i32>, %arg3: tensor<i32>):
    ...
    tensor<5x?x?xf32, #stablehlo.type_extensions<bounds = [?, 5, ?]>>,
    tensor<?x?x?xi32, #stablehlo.type_extensions<bounds = [?, 7, 6]>>
  ) -> (tensor<*xf32>, tensor<*xi32>)
  // CHECK: types0 = tensor<5x?x?xf32, #stablehlo.type_extensions<bounds = [?, 5, 6]>>
  // CHECK-SAME: types1 = tensor<5x?x?xi32, #stablehlo.type_extensions<bounds = [?, 5, 6]>>
...
// CHECK-LABEL: @sort_unknown_rank
func.func @sort_unknown_rank(%input0: tensor<*xf32>, %input1: tensor<*xi32>) {
  %0, %1 = "stablehlo.sort"(%input0, %input1) ({
  ^bb0(%arg0: tensor<f32>, %arg1: tensor<f32>, %arg2: tensor<i32>, %arg3: tensor<i32>):
    %pred = "stablehlo.compare"(%arg0, %arg1) {
      comparison_direction = #stablehlo<comparison_direction GT>
    ...
    "stablehlo.return"(%pred) : (tensor<i1>) -> ()
  }) { dimension = 1 : i64, is_stable = true } : (tensor<*xf32>, tensor<*xi32>) -> (tensor<*xf32>, tensor<*xi32>)
  // CHECK: types0 = tensor<*xf32>
  // CHECK-SAME: types1 = tensor<*xi32>
  %2 = "hlo_test_infer.get_return_types"(%0) : (tensor<*xf32>) -> tensor<*xindex>
  func.return
}
...
func.func @sort_bounds_and_unknown_rank(%input0: tensor<*xf32>, %input1: tensor<5x?x?xi32, #stablehlo.type_extensions<bounds = [?, 7, 6]>>) {
  ...
  // CHECK: types0 = tensor<5x?x?xf32, #stablehlo.type_extensions<bounds = [?, 7, 6]>>
