// RUN: stablehlo-opt %s  -mlir-print-debuginfo -mlir-print-local-scope \
// RUN: | FileCheck %s
// RUN: stablehlo-opt %s  -mlir-print-debuginfo -mlir-print-local-scope \
// RUN: | stablehlo-opt -mlir-print-debuginfo -mlir-print-local-scope \
// RUN: | FileCheck %s
// RUN: stablehlo-opt %s  -mlir-print-debuginfo \
// RUN: | stablehlo-opt -mlir-print-debuginfo -mlir-print-local-scope \
// RUN: | FileCheck %s

// The lit-tests below tests the printing and parsing of the "pretty-printed"
// version of stablehlo.reduce op.

// The test case is eligible for pretty-printing reduce-op.

// CHECK-LABEL:  func @reduce_one_op_all_locs_same
// CHECK-NEXT:     stablehlo.reduce(%arg0 init: %arg1) applies stablehlo.add across dimensions = [1] : (tensor<?x?xf32>, tensor<f32>) -> tensor<?xf32> loc("foo")
...
// CHECK-NEXT:     stablehlo.reduce(%arg0 init: %arg2), (%arg1 init: %arg3)
// CHECK-NOT:     applies
...
// CHECK:          stablehlo.reduce(%arg0 init: %arg1) applies stablehlo.add across dimensions = [1] : (tensor<1x2xcomplex<f32>>, tensor<complex<f32>>) -> tensor<1xcomplex<f32>> loc("foo")
...
// The test case makes sure reduce pretty prints can be parsed, including with
// custom attrs.

// CHECK-LABEL:  func @reduce_parse_pretty_compact
func.func @reduce_parse_pretty_compact(%arg0: tensor<2x64x13xf32>) -> tensor<2x13xf32> {
...
// CHECK-LABEL:  func @reduce_parse_pretty_compact_custom_attr
// CHECK:        {custom_user_attr = 1 : i64}
func.func @reduce_parse_pretty_compact_custom_attr(%arg0: tensor<2x64x13xf32>) -> tensor<2x13xf32> {
  %0 = stablehlo.constant dense<0.000000e+00> : tensor<f32>
  %1 = stablehlo.reduce(%arg0 init: %0) applies stablehlo.add across dimensions = [1] {custom_user_attr = 1 : i64} : (tensor<2x64x13xf32>, tensor<f32>) -> tensor<2x13xf32>
  return %1 : tensor<2x13xf32>
}

// CHECK-LABEL:  func @reduce_parse_pretty_not_compact
func.func @reduce_parse_pretty_not_compact(%arg0: tensor<4x4xf32>, %arg1: tensor<4xf32>) -> tensor<4xf32> {
  %0 = stablehlo.reduce(%arg0 init: %arg1) across dimensions = [0] : (tensor<4x4xf32>, tensor<4xf32>) -> tensor<4xf32>
    reducer(%arg2: tensor<4xf32>, %arg3: tensor<4xf32>)  {
