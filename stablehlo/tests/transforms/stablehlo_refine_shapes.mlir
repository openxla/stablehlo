...
module @refine_call_side_effecting_constant_callee {
  ...
  func.func public @main() -> tensor<?xi32> {
    ...
    // CHECK-SAME: <arg_attrs = [], res_attrs = [{probe.result = "keep"}], no_inline>
    // CHECK-SAME: {probe.marker = "keep"}
    ...
    %1 = call @callee(%0) <arg_attrs = [{probe.argument = "drop"}], no_inline, res_attrs = [{probe.result = "keep"}]> {probe.marker = "keep"} : (tensor<i64>) -> tensor<1xi64>
