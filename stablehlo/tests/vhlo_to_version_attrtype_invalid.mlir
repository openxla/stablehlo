...
vhlo.func @illegal_type_tensor_element(%arg0: !vhlo.tensor<4x16xi16>) -> () {
...
vhlo.func @illegal_type_unranked_tensor_element(%arg0: !vhlo.unranked_tensor<i16>) -> () {
  // expected-error @+1 {{failed to legalize operation 'vhlo.add' that was explicitly marked illegal}}
...
vhlo.func @illegal_type_complex_element(%arg0: !vhlo.tensor<4x16x!vhlo.complex<i16>>) -> () {
  // expected-error @+1 {{failed to legalize operation 'vhlo.add' that was explicitly marked illegal}}
...
vhlo.func @illegal_type_tuple_element(%arg0: !vhlo.tuple<!vhlo.tensor<!vhlo.f32>, !vhlo.tensor<i16>>) -> () {
...
vhlo.func @illegal_type_quantized_storage(%arg0: !vhlo.tensor<!vhlo.quant<i16:!vhlo.f32, 3.400000e+01:16, -128:127, 1>>) -> () {
...
vhlo.func @illegal_type_quantized_expressed(%arg0: !vhlo.tensor<!vhlo.quant<!vhlo.integer<i8>:i16, 3.400000e+01:16, -128:127, 1>>) -> () {
  // expected-error @+1 {{failed to legalize operation 'vhlo.uniform_dequantize' that was explicitly marked illegal}}
...
vhlo.func @vhlo_illegal_attr_dictionary_element(%arg0: !vhlo.tensor<16x16x16x16x!vhlo.f32>) -> () {
  vhlo.return
} {arg_attrs = #vhlo.array<[#vhlo.dict<{#vhlo.string<"stablehlo.self"> = dense<1> : tensor<i16>}>]>}

// -----

// This simulates version validation if a new string attr is introduced.
vhlo.func @vhlo_illegal_attr_symbolref(%arg0: !vhlo.tensor<!vhlo.f32>) -> !vhlo.tensor<!vhlo.f32> {
  // expected-error @+1 {{failed to legalize operation 'vhlo.custom_call_v2' that was explicitly marked illegal}}
  %0 = "vhlo.custom_call_v2"(%arg0) {
    api_version = #vhlo<api_version API_VERSION_ORIGINAL>,
    backend_config = #vhlo.string<"">,
...
// This simulates version validation if a new string attr is introduced.
vhlo.func @vhlo_illegal_attr_float(%arg0: !vhlo.tensor<16x16x16x16x!vhlo.f32>, %arg1: !vhlo.tensor<16x!vhlo.f32>, %arg2: !vhlo.tensor<16x!vhlo.f32>, %arg3: !vhlo.tensor<16x!vhlo.f32>, %arg4: !vhlo.tensor<16x!vhlo.f32>) -> !vhlo.tensor<16x16x16x16x!vhlo.f32> {
